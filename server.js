...
db.serialize(() => {
  ...
  // Safe migration: Check if 'model' column exists before adding
  db.all("PRAGMA table_info(visitor_logs)", (err, columns) => {
    if (err) {
    ...
    if (!hasModel) {
      db.run("ALTER TABLE visitor_logs ADD COLUMN model TEXT", (alterErr) => {
        if (alterErr) {
          if (alterErr.message.includes('duplicate column name')) {
            // Silently ignore if already exists (handles race conditions)
          } else {
            console.error('❌ Error adding "model" column:', alterErr.message);
          }
        } else {
          console.log('✅ Column "model" added to visitor_logs.');
        }
      });
    }
  });
